...
export const validate_token = async (req : express.Request, res : express.Response, next : Function) => {
  ...
  if (!req.header('Bearer')) {
    ...
    try {
      ...
      req.headers['user'] = token.email; req.headers['groups'] = [] //JSON.stringify(['group1','group2']) //TODO: validate JWT here (https://javascript.plainenglish.io/verifying-json-web-tokens-with-express-jwt-1ae147aa9bd3)
      next()
    } 
    catch (err) {
      console.error(err)
